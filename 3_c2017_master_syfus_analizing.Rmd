...
logreg_interaction <- function(emprobs = emprobs) {
 ...
 mutate_at(vars(-c(cd_purp_ec_poly, gend_ec_consec)), funs(round(., digits = 2))) %>% 
 kable()
}
```
...
```{r}
d.an %<>% 
  filter(!is.na(cd_purp_ec) & startedSYFUS == 1)
```
...
```{r}
model <- lm(t1preReasonsIndepcomp2 ~ cd_purp_ec, data = d.an)

em.means <- lm_emmeans(model = model, x_var_ct = "cd_purp_ec")
sum_stats <- em.means %>% as.tibble()
lm_kable_x(lm_emmeans = em.means, x_var = cd_purp_ec)
...
em.means <- lm_emmeans(model = model2, x_var_ct = "cd_purp_ec")
sum_stats <- em.means %>% as.tibble()
lm_kable_x(lm_emmeans = em.means, x_var = cd_purp_ec)
...
**Analyses and plot without covariates:**
```{r}
model <- lm(t1preReasonsIndepcomp2 ~ cd_purp_ec * gend_ec, data = d.an)

lm_contrasts_x_group(model = model, 
                    x_var_ct = "cd_purp_ec", 
                    group_var_ct = "gend_ec",
                    data_d = cohend_ineq, data_d2 = cohend_cd_purp_gend, y_var_d = t1preReasonsIndepcomp2)

sum_stats <- lm_sum_stats_x_group(model = model, 
                     x_var_ct = "cd_purp_ec", 
                     group_var_ct = "gend_ec", group_var = gend_ec, group_levels = group_levels_gend)

lm_kable_x_group(data = sum_stats, x_var = cd_purp_ec, group_var = gend_ec)

summary(model)

lm_emmeans_contrast_x(model = model, x_var_ct = "cd_purp_ec")
lm_emmeans_contrast_x(model = model, x_var_ct = "gend_ec")

em.means <- lm_emmeans_x_group(model = model, x_var_ct = "cd_purp_ec", group_var_ct = "gend_ec")
lm_interaction(emmeans = em.means)

plot_bar_x_y_group(data = sum_stats, x_var = cd_purp_ec, x_labels = labels_cd_purp, 
                   x_color = color_cd_purp,
                   y_var = emmean, labs_y = labs_y,
                   group_var = gend_ec, group_names = gend_names,
                   scale = scale_y_5, coord_cartesian = coord_cartesian_5, mean_round = mean_round3)
```
...
lm_emmeans_contrast_x(model = model2, x_var_ct = "cd_purp_ec")
lm_emmeans_contrast_x(model = model2, x_var_ct = "gend_ec")

em.means <- lm_emmeans_x_group(model = model2, x_var_ct = "cd_purp_ec", group_var_ct = "gend_ec")
lm_interaction(emmeans = em.means)
...
**Analyses and plot without covariates:**
```{r}
model <- lm(t1preReasonsInterdepcomp2 ~ cd_purp_ec, data = d.an)
...
**Analyses and plot without covariates:**
```{r}
model <- lm(t1preReasonsInterdepcomp2 ~ cd_purp_ec * gend_ec, data = d.an)
...
```{r}
model <- glm(t1manipcheck_binary_num ~ cd_purp_ec, data = d.an, family = binomial, na.action = na.omit)

em.probs <- logreg_emprobs_x(model = model, x_var_ct = "cd_purp_ec")

em.probs.table <- logreg_emprobs_table_x(emprobs = em.probs)
em.probs.table

contrast_logreg_x(model = model, x_var_ct = "cd_purp_ec")

labs_y <- labs(x = "", y = "% Pass Manipulation Check")

plot_bar_x_y_pct(data = em.probs.table, x_var = cd_purp_ec, x_labels = labels_cd_purp, 
                   x_color = color_cd_purp,
...
em.probs <- logreg_emprobs_x(model = model2, x_var_ct = "cd_purp_ec")

em.probs.table <- logreg_emprobs_table_x(emprobs = em.probs)

contrast_logreg_x(model = model2, x_var_ct = "cd_purp_ec")
...
**Plot and model without covariates:**
```{r}
model <- glm(t1manipcheck_binary_num ~ cd_purp_ec*gend_ec, data = d.an, family = binomial, na.action = na.omit)

em.probs <- logreg_emprobs_x_group(model = model, x_var_ct = "cd_purp_ec", group_var_ct = "gend_ec")

em.probs.table <- logreg_emprobs_table_x_group(emprobs = em.probs, group_var = gend_ec, group_levels = group_levels_gend)

em.probs.table

contrast_logreg_x_group(model = model, x_var = cd_purp_ec, x_var_ct = "cd_purp_ec", 
                            group_var = gend_ec, group_var_ct = "gend_ec")

logreg_interaction(em.probs)

plot_bar_x_y_group_pct(data = em.probs.table, x_var = cd_purp_ec, x_labels = labels_cd_purp, 
                   x_color = color_cd_purp,
                   y_var = prob*100, labs_y = labs_y,
                   group_var = gend_ec, group_names = gend_names,
                   scale = scale_y_100, coord_cartesian = coord_cartesian_100, mean_round = mean_round_pct)
...
em.probs <- logreg_emprobs_x_group(model = model2, x_var_ct = "cd_purp_ec", group_var_ct = "gend_ec")

em.probs.table <- logreg_emprobs_table_x_group(emprobs = em.probs, group_var = gend_ec, group_levels = group_levels_gend)

em.probs.table

contrast_logreg_x_group(model = model2, x_var = cd_purp_ec, x_var_ct = "cd_purp_ec", 
                            group_var = gend_ec, group_var_ct = "gend_ec")
...
```{r}
model <- lm(t1opportunty_independent_comp2 ~ cd_purp_ec, data = d.an)
...
```{r}
model <- lm(t1opportunty_independent_comp2 ~ cd_purp_ec * gend_ec, data = d.an)
...
```{r}
model <- lm(t1opportunty_interdependent_comp2 ~ cd_purp_ec, data = d.an)
...
```{r}
model <- lm(t1opportunty_interdependent_comp2 ~ cd_purp_ec * gend_ec, data = d.an)
...
```{r}
model <- lm(belong_comp6 ~ cd_purp_ec, data = d.an)

sum_stats <- emmeans(model, ~ cd_purp_ec) %>% as.tibble()
sum_stats %>% mutate_at(vars(-c(cd_purp_ec)), funs(round(., 2))) %>% kable()
cohend_cd_purp_all %>% filter(rowname == "belong_comp6")
lm_emmeans(model = model, x_var_ct = "cd_purp_ec") %>% pairs
summary(model)
...
sum_stats <- emmeans(model2, ~ cd_purp_ec) %>% as.tibble()
sum_stats %>% mutate_at(vars(-c(cd_purp_ec)), funs(round(., 2))) %>% kable()
cohend_cd_purp_all %>% filter(rowname == "belong_comp6")
lm_emmeans(model = model2, x_var_ct = "cd_purp_ec") %>% pairs
summary(model2)
...
Analyses and plot without covariates:
```{r}
model <- lm(belong_comp6 ~ cd_purp_ec * gend_ec, data = d.an)

# is there inequality?
emmeans(model, ~ gend_ec | cd_purp_ec) %>% pairs 
cohend_ineq %>% select(belong_comp6) %>% rename(purpose_d_ineq_ctl = belong_comp6) # cohen's d

# is there a treatment effect?
sum_stats <- emmeans(model, ~ cd_purp_ec | gend_ec) 
pairs(sum_stats) 
cohend_cd_purp_gend %>% select(gender, belong_comp6) # cohen's d
sum_stats %<>% as.tibble() %>% mutate(gend_ec = factor(gend_ec, levels=c('m','f')))
sum_stats %>% mutate_at(vars(-c(cd_purp_ec, gend_ec)), funs(round(., 2))) %>% kable()
...
# is there inequality?
emmeans(model2, ~ gend_ec | cd_purp_ec) %>% pairs 
cohend_ineq %>% select(belong_comp6) %>% rename(d_ineq_ctl = belong_comp6) # cohen's d

# is there a treatment effect?
sum_stats <- emmeans(model2, ~ cd_purp_ec | gend_ec) 
pairs(sum_stats) 
cohend_cd_purp_gend %>% select(gender, belong_comp6) # cohen's d
sum_stats %<>% as.tibble() %>% mutate(gend_ec = factor(gend_ec, levels=c('m','f')))
sum_stats %>% mutate_at(vars(-c(cd_purp_ec, gend_ec)), funs(round(., 2))) %>% kable()
...
model <- lm(job_applylearning ~ cd_purp_ec, data = d.an)

sum_stats <- emmeans(model, ~ cd_purp_ec) 
...
Analyses and plot without covariates:
```{r}
model <- lm(job_applylearning ~ cd_purp_ec * gend_ec, data = d.an)
...
emmeans(model, ~ cd_purp_ec | gend_ec) %>% pairs
model <- lm(job_applylearning ~ cd_purp_ec, data = d.an %>% filter(gend_ec == "f"))
model %>% summary
```
...
Plot and model without covariates:
```{r}
model <- glm(honors_yes_use_this ~ cd_purp_ec, data = d.an, family = binomial, na.action = na.omit)
...
Plot and model without covariates:
```{r}
model <- glm(honors_yes_use_this ~ cd_purp_ec*gend_ec, data = d.an, family = binomial, na.action = na.omit)
...
# β3 is the difference between the log-odds ratio comparing
...
**Updated code:**
```{r}
model <- lm(collexp_comp4 ~ cd_purp_ec, data = d.an)
...
sum_stats <- lm_sum_stats_x(model = model, x_var_ct = "cd_purp_ec")
lm_kable_x(data = sum_stats, x_var = cd_purp_ec)
...
```{r}
model <- lm(collexp_comp4 ~ cd_purp_ec * gend_ec, data = d.an)
...
# emmeans(model, list(pairwise ~ cd_purp_ec | gend_ec, pairwise ~ gend_ec | cd_purp_ec))
```
...
```{r}
model <- lm(qualitiesind_comp2 ~ cd_purp_ec, data = d.an)
...
# sum_stats <- lm_sum_stats_x(model = model2, x_var_ct = "cd_purp_ec")
# lm_kable_x(data = sum_stats, x_var = cd_purp_ec)
...
```{r}
model <- lm(qualitiesind_comp2 ~ cd_purp_ec * gend_ec, data = d.an)
...
```{r}
model <- lm(qualitiesinter_comp4 ~ cd_purp_ec, data = d.an)
...
```{r}
model <- lm(qualitiesinter_comp4 ~ cd_purp_ec * gend_ec, data = d.an)
...
```{r}
model <- lm(potsuc_comp2 ~ cd_purp_ec, data = d.an)
...
```{r}
model <- lm(potsuc_comp2 ~ cd_purp_ec * gend_ec, data = d.an)
...
Plot and model without covariates:
```{r}
model <- glm(mentor_yes ~ cd_purp_ec, data = d.an, family = binomial, na.action = na.omit)
...
d.pred <- predicted_data_model_x_y(model = model, 
    data = d.an, x_var = cd_purp_ec, y_var = mentor_yes)

labs_y <- labs(x = "", y = "% Has a Mentor")

plot_bar_x_y_pct(data = d.pred, x_var = cd_purp_ec, x_labels = labels_cd_purp, 
                   x_color = color_cd_purp,
...
Plot and model without covariates:
```{r}
model <- glm(mentor_yes ~ cd_purp_ec*gend_ec, data = d.an, family = binomial, na.action = na.omit)
...
# emmeans(model, pairwise ~ cd_purp_ec | gend_ec, type = "response")
# emmip(model, cd_purp_ec ~ gend_ec, type = "response")
...
contrast_logreg_x_group(model = model, x_var_ct = "cd_purp_ec", group_var_ct = "gend_ec")

d.pred <- 
  predicted_data_model_x_y_group(
    model = model, 
    data = d.an, x_var = cd_purp_ec, y_var = mentor_yes, 
    group_var = gend_ec, group_levels = group_levels_gend)

plot_bar_x_y_group_pct(data = d.pred, x_var = cd_purp_ec, x_labels = labels_cd_purp, 
                   x_color = color_cd_purp,
                   y_var = predicted.pct*100, labs_y = labs_y,
                   group_var = gend_ec, group_names = gend_names,
                   scale = scale_y_100, coord_cartesian = coord_cartesian_100, mean_round = mean_round_pct)
...
contrast_logreg_x_group(model = model2, x_var_ct = "cd_purp_ec", group_var_ct = "gend_ec")
```

```{r eval=FALSE, include=FALSE}
model <- glm(mentor_yes ~ cd_purp_ec*gend_ec, data = d.an, family = binomial, na.action = na.omit)

emmeans(model, "cd_purp_ec", by = "gend_ec", type = "response")
emmeans(model, "cd_purp_ec", by = "gend_ec", type = "response") %>% pairs

model %>% summary

emmeans(model,  "gend_ec", by = "cd_purp_ec", type = "response")
emmeans(model,  "gend_ec", by = "cd_purp_ec", type = "response") %>% pairs

# emmeans(model, ~ cd_purp_ec | gend_ec, type = "response")
...
plot_bar_x_y_group_pct(data = sum_stats, x_var = cd_purp_ec, x_labels = labels_cd_purp, 
                   x_color = color_cd_purp,
                   y_var = prob*100, labs_y = labs_y,
                   group_var = gend_ec, group_names = gend_names,
                   scale = scale_y_100, coord_cartesian = coord_cartesian_100, mean_round = mean_round_pct2)

pairs(emmeans(model, "cd_purp_ec", by = "gend_ec", type = "response")) 
```

```{r eval=FALSE, include=FALSE}
model <- glm(mentor_yes ~ cd_purp_ec2*gend_ec2, data = d.an, family = binomial, na.action = na.omit)

emmeans(model, "cd_purp_ec2", by = "gend_ec2", type = "response")
emmeans(model, "cd_purp_ec2", by = "gend_ec2", type = "response") %>% pairs

model %>% summary

model <- glm(mentor_yes ~ cd_purp_ec2*gend_ec2 + sattoact + hsrank, data = d.an, family = binomial, na.action = na.omit)
...
# d.an %>% filter(!is.na(cd_purp_ec)) %>% select(cd_purp, cd_purp_ec, gend, gend_ec) %>% str

# d.an %>% count(cd_purp, cd_purp_ec)
```

```{r eval=FALSE, include=FALSE}
model <- glm(mentor_yes ~ cd_purp_ec*gend_ec, data = d.an, family = binomial, na.action = na.omit)

em.probs <- logreg_emprobs(model = model, x_var_ct = "cd_purp_ec", group_var_ct = "gend_ec")

# odds.ratio1 <- logreg_oddsratio(em.probs = em.probs1, group_var = gend_ec, group_var_ct = "gend_ec")
# odds.ratio2 <- logreg_oddsratio(em.probs = em.probs2, group_var = cd_purp_ec, group_var_ct = "cd_purp_ec")

# contrast_logreg_x_group(model = model, x_var_ct = "cd_purp_ec", group_var_ct = "gend_ec")

contrast_logreg_x_group(model = model, x_var = cd_purp_ec, x_var_ct = "cd_purp_ec", group_var = gend_ec, group_var_ct = "gend_ec")

model <- glm(mentor_yes ~ cd_purp_ec2*gend_ec2 + sattoact + hsrank, data = d.an, family = binomial, na.action = na.omit)

emprobs <- emmeans(model, "cd_purp_ec2", by = "gend_ec2", type = "response")
...
emprobs %>% 
  ...
  ungroup() %>% 
  left_join(emprobs %>% pairs %>% as.tibble(), by = "gend_ec2")

contrast_logreg_x_group(model = model, x_var_ct = "cd_purp_ec2", group_var_ct = "gend_ec2")
...
Plot without covariates:
```{r}
model <- lm(purpose_comp2 ~ cd_purp_ec, data = d.an)
...
Plot without covariates:
```{r}
model <- lm(purpose_comp2 ~ cd_purp_ec * gend_ec, data = d.an)
...
Plot without covariates:
```{r}
model <- lm(lifesat ~ cd_purp_ec, data = d.an)
...
Plot without covariates:
```{r}
model <- lm(lifesat ~ cd_purp_ec * gend_ec, data = d.an)
...
Plot without covariates:
```{r}
model <- lm(happy_comp4 ~ cd_purp_ec, data = d.an)
...
sum_stats <- emmeans(model2, ~ cd_purp_ec) %>% as.tibble()
emmeans(model2, ~ cd_purp_ec) %>% pairs
sum_stats %>% mutate_at(vars(-c(cd_purp_ec)), funs(round(., 2))) %>% kable()
cohend_cd_purp_all %>% filter(rowname == "happy_comp4")
lm_emmeans(model = model2, x_var_ct = "cd_purp_ec") %>% pairs
summary(model2)
...
Plot without covariates:
```{r}
model <- lm(happy_comp4 ~ cd_purp_ec * gend_ec, data = d.an)
...
Plot without covariates:
```{r}
model <- lm(healthmosoverall_comp5 ~ cd_purp_ec, data = d.an)
...
Plot without covariates:
```{r}
model <- lm(healthmosoverall_comp5 ~ cd_purp_ec * gend_ec, data = d.an)
...
Plot without covariates:
```{r}
model <- lm(stressdaily ~ cd_purp_ec, data = d.an)
...
Plot without covariates:
```{r}
model <- lm(stressdaily ~ cd_purp_ec * gend_ec, data = d.an)
...
Plot without covariates:
```{r}
model <- lm(pss_comp4 ~ cd_purp_ec, data = d.an)
...
Plot without covariates:
```{r}
model <- lm(pss_comp4 ~ cd_purp_ec * gend_ec, data = d.an)
...
Plot without covariates:
```{r}
model <- lm(overallmentalhealth_comp5_z ~ cd_purp_ec, data = d.an)
...
Plot without covariates:
```{r}
model <- lm(overallmentalhealth_comp5_z ~ cd_purp_ec * gend_ec, data = d.an)
...
Plot without covariates:
```{r}
model <- lm(Y1_Sp_CumGPA ~ cd_purp_ec, data = d.an)
...
Plot without covariates:
```{r}
model <- lm(Y1_Sp_CumGPA ~ cd_purp_ec * gend_ec, data = d.an)
...
model <- lm(activism_z_comp2 ~ cd_purp_ec, data = d.an)
...
Plot without covariates:
```{r}
model <- lm(activism_z_comp2 ~ cd_purp_ec * gend_ec, data = d.an)
...
model <- lm(activism_engaged ~ cd_purp_ec, data = d.an)
...
Plot without covariates:
```{r}
model <- lm(activism_engaged ~ cd_purp_ec * gend_ec, data = d.an)
